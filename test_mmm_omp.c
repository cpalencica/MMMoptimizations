...
void block_mmm_ijk(matrix_ptr a, matrix_ptr b, matrix_ptr c, int block_size);
...
double wakeup_delay()
{
    ...
    while (meas < 1.0)
    {
        for (i = 1; i < j; i++)
        {
            /* This iterative calculation uses a chaotic map function,
               specifically the complex quadratic map (as in Julia and
               Mandelbrot sets), which is unpredictable enough to prevent
...
int main(int argc, char* argv[])
{
    ...
    printf("row_len, blocked ijk with omp\n");
    {
        int i, j;
...
long int get_matrix_row_length(matrix_ptr m) { return m->len; }
...
void block_mmm_ijk(matrix_ptr a, matrix_ptr b, matrix_ptr c, int block_size)
{
    ...
    data_t sum;
    int en = block_size * (length / block_size);
    printf("EN: %d\n", en);

#pragma omp parallel for private(i, j, k, kk, sum) schedule(static)
